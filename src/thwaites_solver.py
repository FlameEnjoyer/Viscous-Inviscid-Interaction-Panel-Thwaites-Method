...
class ThwaitesSolver:
    ...
    def calculate_properties(self, s, Ue, nu, rho):
        ...
        # IMPROVED: Post-separation handling
        if sep_idx < len(lam):
            ...
            if sep_idx > 1:
                dtheta = (theta[sep_idx-1] - theta[sep_idx-2]) * 0.5  # Reduced growth
                for i in range(sep_idx, len(lam)):
                    theta[i] = theta[i-1] + dtheta
            else:
                # If separation at first points, use constant theta
                for i in range(sep_idx, len(lam)):
                    theta[i] = theta[sep_idx-1] if sep_idx > 0 else 0.001

        # Apply max H limit everywhere (cap unphysical values)
        H_lam = np.clip(H_lam, 1.4, 4.0)  # Physical limits for H
